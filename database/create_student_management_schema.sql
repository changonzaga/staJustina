...
-- Function to generate student account number
DELIMITER //
CREATE FUNCTION generate_student_account_number()
RETURNS VARCHAR(20)
READS SQL DATA
DETERMINISTIC
BEGIN
    ...
    SELECT COALESCE(MAX(CAST(SUBSTRING(student_account_number, 4) AS UNSIGNED)), 0) + 1
    INTO sequence_num
    FROM students
    WHERE student_account_number LIKE CONCAT('STA', year_suffix, '%');
    
    SET new_number = CONCAT('STA', year_suffix, LPAD(sequence_num, 6, '0'));
...
-- Ensure only one primary contact per student in family_info
ALTER TABLE student_family_info ADD CONSTRAINT chk_one_primary_contact 
CHECK (is_primary_contact = FALSE OR 
       (SELECT COUNT(*) FROM student_family_info sf2 
        WHERE sf2.student_id = student_family_info.student_id 
        AND sf2.is_primary_contact = TRUE 
        AND sf2.id != student_family_info.id) = 0);

-- Ensure only one primary address per student
ALTER TABLE student_address ADD CONSTRAINT chk_one_primary_address 
CHECK (is_primary = FALSE OR 
       (SELECT COUNT(*) FROM student_address sa2 
        WHERE sa2.student_id = student_address.student_id 
        AND sa2.is_primary = TRUE 
        AND sa2.id != student_address.id) = 0);
...
-- Auto-generate student account number on insert
DELIMITER //
CREATE TRIGGER tr_students_before_insert
BEFORE INSERT ON students
FOR EACH ROW
BEGIN
    IF NEW.student_account_number IS NULL OR NEW.student_account_number = '' THEN
        SET NEW.student_account_number = generate_student_account_number();
    END IF;
END//
DELIMITER ;
...
AFTER UPDATE ON student_notifications
FOR EACH ROW
BEGIN
